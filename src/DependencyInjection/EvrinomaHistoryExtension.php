...
use Evrinoma\HistoryBundle\Factory\History\Factory as HistoryFactory;
use Evrinoma\HistoryBundle\Mediator\QueryMediatorInterface;
use Evrinoma\HistoryBundle\Repository\History\HistoryCommandRepositoryInterface;
...
class EvrinomaHistoryExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\HistoryBundle\Entity';
    public const MODEL = 'Evrinoma\HistoryBundle\Model';
    public const ENTITY_FACTORY_HISTORY = HistoryFactory::class;
    public const ENTITY_BASE_HISTORY = BaseHistory::class;
    public const DTO_BASE_HISTORY = HistoryApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_HISTORY !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, HistoryPass::HISTORY_CONSTRAINT)   :
                    $definition->addTag(HistoryPass::HISTORY_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([HistoryCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([HistoryQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaHistoryBundle::BUNDLE;
    }
}
